...
@Singleton
class EnrolmentStoreProxyConnector @Inject()(httpClient: HttpClientV2,
                                                                       val metrics: Metrics,
  ...
  def getPrincipalGroupIdFor(
    arn: Arn)(implicit
    rh: RequestHeader): Future[String] = {
  ...
  )(implicit rh: RequestHeader): Future[Set[String]] = {
  ...
  def getAgentReferenceNumberFor(
    ...
    monitor(s"ConsumedAPI-ES-getEnrolmentsForGroupId-$groupId-GET") {
      httpClient
        .get(url"$espBaseUrl/enrolment-store-proxy/enrolment-store/groups/$groupId/enrolments?type=principal&service=HMRC-AS-AGENT")
        .execute[HttpResponse]
        .map { response =>
         response.status match {
          //TODO: improve error handling as described above and don't use NO_CONTENT for the case of no enrolments
