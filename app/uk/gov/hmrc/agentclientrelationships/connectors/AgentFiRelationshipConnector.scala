...
class AgentFiRelationshipConnector @Inject()(
  ...
  def getRelationship(arn: Arn, service: String, clientId: String)(implicit
    rh: RequestHeader
      ...
      httpClient
        .get(
          afiRelationshipUrl(arn, service, clientId)
        )
  ...
  def getInactiveRelationships(implicit
    rh: RequestHeader
  ): Future[Seq[InactiveRelationship]] = monitor(s"ConsumedAPI-AgentFiRelationship-PERSONAL-INCOME-RECORD-GET") {
      ...
      httpClient
        ...
        //TODO: change the underlying endpoint to return emtpy list instead of NOT_FOUND. Easier to implement and no problem when to distinguish between URL not found and records are not there
        .execute[Option[Seq[InactiveRelationship]]]
        .map(_.fold(Seq())(identity))
}
  ...
  def createRelationship(
                          ...
                          service: String,
                          clientId: String,
                          acceptedDate: LocalDateTime)(implicit
    rh: RequestHeader
  ): Future[Unit] = {
  ...
  //TODO: Verify the boolean is really needed. It seems that NotFound is transformed into false, which is then transformed into NotFound ...
  =
  ...
  def findIrvActiveRelationshipForClient(
    ...
    EitherT
      .fromOptionF(
        fopt = findIrvRelationshipForClient(nino),
