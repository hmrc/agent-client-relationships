...
case class Authority(enrolmentsUrl: URL)
...
class AuthConnector @Inject()(@Named("auth-baseUrl") baseUrl: URL, httpGet: HttpGet) {
  ...
  def currentAuthority()(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[Option[Authority]] = {
    ...
    httpGetAs[JsValue](authorityUrl) flatMap { r =>
      for {
        enrolmentsUrl <- Future successful new URL(authorityUrl, enrolmentsRelativeUrl(r))
  ...
  def currentArn(enrolmentUrl: URL)(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[Option[Arn]] =
    enrolments(enrolmentUrl).map(_.arnOption)

  def currentMtdItId(enrolmentsUrl: URL)(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[Option[MtdItId]] =
  ...
  private def httpGetAs[T](relativeUrl: URL)(implicit rds: HttpReads[T], hc: HeaderCarrier, ec: ExecutionContext): Future[T] =
    httpGet.GET[T](relativeUrl.toString)(rds, hc)
}
