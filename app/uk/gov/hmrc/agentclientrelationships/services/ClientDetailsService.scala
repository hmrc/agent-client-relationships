...
extends Logging {
    ...
    clientDetailsConnector
      ...
      .map {
        ...
        case Right(details @ VatCustomerDetails(_, _, _, Some(regDate), _)) =>
          val clientName = details
            .tradingName
            .getOrElse(details.organisationName.getOrElse(details.individual.get.name))
          val clientStatus =
  ...
  private def getTrustClientDetails(
    ...
    clientDetailsConnector
      ...
      .map {
        case Right(name) => Right(ClientDetailsResponse(name, None, None, Seq(), None))
        case Left(err)   => Left(err)
      }
    ...
    clientDetailsConnector
      ...
      .map {
        case Right(details @ ItsaCitizenDetails(_, _, Some(dateOfBirth), _)) =>
          details.name match {
            case Some(name) => Right(ClientDetailsResponse(name, None, None, Seq(dateOfBirth.toString), Some(Date)))
            case None =>
