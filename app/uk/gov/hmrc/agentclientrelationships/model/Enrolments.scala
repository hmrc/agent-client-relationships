...
case class Enrolments(enrolments: Set[AuthEnrolment]) {
  ...
  def mtdItIdOption: Option[MtdItId] = asClientEnrolment.flatMap(_.identifier("AgentReferenceNumber")).map(MtdItId.apply)

  private def asAgentEnrolment: Option[AuthEnrolment] = getEnrolment("HMRC-AS-AGENT")

  private def asClientEnrolment: Option[AuthEnrolment] = getEnrolment("HMRC-MTD-IT")
