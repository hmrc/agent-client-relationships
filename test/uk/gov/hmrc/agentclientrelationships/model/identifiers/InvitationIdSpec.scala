...
import uk.gov.hmrc.agentclientrelationships.model.identifiers.InvitationId.byteToBitsLittleEndian
import uk.gov.hmrc.agentclientrelationships.model.identifiers.InvitationId.bytesTo5BitNums
import uk.gov.hmrc.agentclientrelationships.model.identifiers.InvitationId.checksumDigits
import uk.gov.hmrc.agentclientrelationships.model.identifiers.InvitationId.to5BitAlphaNumeric
import uk.gov.hmrc.agentclientrelationships.model.identifiers.InvitationId.to5BitNum

import java.nio.charset.StandardCharsets
import java.time.LocalDate

class InvitationIdSpec
extends AnyFlatSpec
with Matchers {

  val invWithoutPrefix =
    (prefix: Char) =>
      InvitationId.create(
        "myAgency",
        "clientId",
  ...
  "byteToBitsLittleEndian" should "return little endian bit sequences (LSB first) for signed bytes" in {
    byteToBitsLittleEndian(0) shouldBe Seq(
      false,
      false,
      false,
      false,
    ...
    byteToBitsLittleEndian(1) shouldBe Seq(
      true,
      false,
      false,
      false,
    ...
    byteToBitsLittleEndian(2) shouldBe Seq(
      false,
      true,
      false,
      false,
    ...
    byteToBitsLittleEndian(3) shouldBe Seq(
      true,
      true,
      false,
      false,
    ...
    byteToBitsLittleEndian(127) shouldBe Seq(
      true,
      true,
      true,
      true,
    ...
    byteToBitsLittleEndian(-128) shouldBe Seq(
      false,
      false,
      false,
      false,
  ...
  it should "provide checksum for many bytes" in {
    CRC10.calculate(Array[Byte](
      ...
      7,
      8
    )) shouldBe CRC10.calculate(Array[Byte](
      1,
      2,
      3,
      4,
