...
import uk.gov.hmrc.agentmtdidentifiers.model.Service.{MtdIt, MtdItSupp, PersonalIncomeRecord, Vat}
...
class AuthorisationAcceptServiceSpec
  ...
  "accept" when {
    ...
    s"called with a general case invitation" should {
      "create a relationship" should also {
    ...
    "called with a HMRC-MTD-IT invitation" should {
      "check for and deauth an existing itsa relationship with the same agent" should also {
        "check for and deauth an existing partial auth and create a relationship" should also {
          "update the invitation's status, check for and deauth previously accepted invitations and send a confirmation email" in {
            mockDeleteSameAgentRelationship(MtdIt.id, testArn.value, Some(testMtdItId.value), testNino.nino)(
      ...
      "check for and deauth an existing itsa relationship with the same agent" should also {
        "create a relationship" should also {
          "update the invitation's status and send a confirmation email" in {
            mockDeleteSameAgentRelationship(MtdItSupp.id, testArn.value, Some(testMtdItId.value), testNino.nino)(
              Future.successful(true)
            )
            mockCreateRelationship(testArn, itsaEnrolment)(Future.successful(Some(DbUpdateSucceeded)))
