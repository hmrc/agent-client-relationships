...
trait DataStreamStub extends Eventually {
  ...
  def verifyAuditRequestSent(
    ...
    verify(
      count,
      postRequestedFor(urlPathEqualTo(auditUrl)).withRequestBody(similarToJson(s"""{
  ...
  def verifyAuditRequestNotSent(event: AgentClientRelationshipEvent) = eventually {
    verify(
      0,
      postRequestedFor(urlPathEqualTo(auditUrl)).withRequestBody(similarToJson(s"""{
          |  "auditSource": "agent-client-relationships",
