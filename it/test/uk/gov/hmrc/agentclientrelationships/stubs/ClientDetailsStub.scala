...
trait ClientDetailsStub {
  ...
  def givenItsaDesignatoryDetailsExists(nino: String): StubMapping = stubFor(
    get(urlEqualTo(s"/citizen-details/$nino/designatory-details")).willReturn(aResponse().withBody(s"""
                                                                                                      |{
                                                                                                      |  "address": {
                                                                                                      |    "postcode": "AA1 1AA"
                                                                                                      |  }
  ...
  def givenItsaCitizenDetailsExists(nino: String): StubMapping = stubFor(
    get(urlEqualTo(s"/citizen-details/nino/$nino")).willReturn(aResponse().withBody(s"""
                                                                                       |{
                                                                                       |  "name": {
                                                                                       |    "current": {
                                                                                       |      "firstName": "Matthew",
  ...
  def givenVatCustomerInfoExists(vrn: String): StubMapping = stubFor(
    get(urlEqualTo(s"/vat/customer/vrn/$vrn/information")).willReturn(aResponse().withBody(s"""
                                                                                              |{
                                                                                              |  "approvedInformation": {
                                                                                              |    "customerDetails": {
  ...
  def givenCgtDetailsExist(cgtRef: String): StubMapping = stubFor(
    get(urlEqualTo(s"/subscriptions/CGT/ZCGT/$cgtRef")).willReturn(aResponse().withBody(s"""
                                                                                           |{
                                                                                           |  "subscriptionDetails": {
                                                                                           |    "typeOfPersonDetails": {
  ...
  def givenPptDetailsExist(pptRef: String): StubMapping = stubFor(
    get(urlEqualTo(s"/plastic-packaging-tax/subscriptions/PPT/$pptRef/display")).willReturn(aResponse().withBody(s"""
                                                                                                                    |{
                                                                                                                    |  "legalEntityDetails": {
  ...
  def givenCbcDetailsExist(isGBUser: Boolean = true): StubMapping = stubFor(
    post(urlEqualTo("/dac6/dct50d/v1")).willReturn(aResponse().withBody(s"""{
  ...
  def givenCbcDetailsError(status: Int): StubMapping = stubFor(
    post(urlEqualTo("/dac6/dct50d/v1")).willReturn(aResponse().withStatus(status))
  )

  def givenPillar2DetailsExist(plrId: String): StubMapping = stubFor(
    get(urlEqualTo(s"/pillar2/subscription/$plrId")).willReturn(aResponse().withBody(s"""{
                                                                                        |  "success": {
                                                                                        |    "upeDetails": {
                                                                                        |      "organisationName": "CFG Solutions",
