        ...
        (obj \ name).asOpt[T] match {
          case Some(value) =>
            if (matcher != null)
              ...
              MatchResult(true, "", s"JSON have property `$name`")
          case _ =>
            MatchResult(
              false,
              s"JSON should have property `$name` of type ${classTag.runtimeClass.getSimpleName}, but had only ${obj
                .fields
      ...
      override def apply(obj: JsObject): MatchResult =
        (obj \ name).asOpt[JsArray] match {
          ...
          case _ =>
            MatchResult(
              false,
              s"JSON should have array property `$name` of item type ${classTag
                .runtimeClass
                .getSimpleName}, but had only ${obj.fields.map(f => s"${f._1}:${f._2.getClass.getSimpleName}").mkString(", ")}",
              ""
            )
        }
    }
    ...
    new Matcher[JsObject] {
      override def apply(obj: JsObject): MatchResult =
        (obj \ name).asOpt[JsValue] match {
          case Some(value) => MatchResult(false, s"JSON should not have property `$name` but we got value $value", s"")
          case None        => MatchResult(true, "", s"JSON does not have property `$name`")
        }
    }
