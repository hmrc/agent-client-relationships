...
class RelationshipCopyRecordRepositoryISpec extends UnitSpec with MongoApp with OneAppPerSuite {
  ...
  "RelationshipCopyRecordRepository" should {
    "create, find and update and remove a record" in {
      val relationshipCopyRecord = RelationshipCopyRecord("TARN0000001", "101747696", "VRN", None, DateTime.now(DateTimeZone.UTC), Some(SyncStatus.Failed), Some(SyncStatus.Failed))
      ...
      findResult3 shouldBe Some(relationshipCopyRecord.copy(syncToETMPStatus = Some(SyncStatus.Success), syncToESStatus = Some(SyncStatus.Success)))
