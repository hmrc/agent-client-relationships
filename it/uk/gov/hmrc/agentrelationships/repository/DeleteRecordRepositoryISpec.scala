...
class DeleteRecordRepositoryISpec extends UnitSpec with MongoApp with OneAppPerSuite {
  ...
  "DeleteRecordRepository" should {
    "create, find and update and remove a record" in {
      val deleteRecord = DeleteRecord("TARN0000001", "101747696", "VRN", DateTime.now(DateTimeZone.UTC), Some(SyncStatus.Failed), Some(SyncStatus.Failed))
      ...
      findResult3 shouldBe Some(deleteRecord.copy(syncToETMPStatus = Some(SyncStatus.Success), syncToESStatus = Some(SyncStatus.Success)))
